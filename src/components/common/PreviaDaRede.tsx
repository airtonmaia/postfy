...
export const PreviaDaRede: React.FC<{ dados: DadosDaPrevia; className?: string }> = ({
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <Tabs value={canalAtivo} onValueChange={(v) => setCanalAtivo(v as JobPlatform)}>
        <TabsList aparencia="painel" className="bg-transparent dark:bg-transparent px-0 rounded-none">
          {canais.map((canal) => {
            const Icone = ICONE_DA_REDE[canal];
            return (
              <TabsTrigger key={canal} value={canal} className="px-3">
      ...
      {quadros.length > 1 && (
        <Tabs
          value={String(indice)}
          onValueChange={(v) => setIndice(Number(v))}
        >
          <TabsList aparencia="segmentado" className="w-full">
            {quadros.map((q, i) => (
              <TabsTrigger key={q.chave} value={String(i)} className="flex-1">
                {q.rotulo}
              </TabsTrigger>
            ))}
          </TabsList>
