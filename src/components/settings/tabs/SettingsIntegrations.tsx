...
/**
 * Esta tela já anunciou "Conectado" para integrações que não existiam, e
 * depois passou a descrever a arquitetura anterior: falava de um "Servidor
 * Orquesia" que foi removido, mandava rodar um schema.sql que não existe mais
 * e dizia que os arquivos ficavam no navegador.
 *
 * O padrão é sempre o mesmo: uma lista escrita à mão não acompanha o código.
 * Agora o que depende do servidor é perguntado ao servidor (/api/status), e
 * só o que é fato do próprio repositório fica fixo aqui.
 */
...
export const SettingsIntegrations: React.FC = () => {
  ...
  const integracoes: { nome: string; status: StatusIntegracao | null; desc: string }[] = [
    ...
    },
    {
      ...
      desc: 'Exige OAuth com Meta, LinkedIn e TikTok, fila de disparo e aprovação de app na Meta. Em construção.',
    },
    {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              O que depende de chave é consultado no servidor, não escrito aqui à mão.
            </p>
          </div>
        </div>
